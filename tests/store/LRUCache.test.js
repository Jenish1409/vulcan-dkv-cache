...
const LRUCache_1 = require("@/store/LRUCache");
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/** Build a cache and register automatic cleanup after each test. */
function makeCache(maxCapacity, sweepIntervalMs = 0 // disable active sweep by default — tests drive time manually
) {
    return new LRUCache_1.LRUCache({ maxCapacity, sweepIntervalMs });
}
// ---------------------------------------------------------------------------
// a) Basic SET / GET / DELETE
// ---------------------------------------------------------------------------
...
describe("TTL expiry (lazy)", () => {
    ...
    test("key with TTL is retrievable before it expires", () => {
        ...
        jest.advanceTimersByTime(4999); // 4.999 s — still alive
    ...
    test("key with TTL returns null after expiry (lazy check on GET)", () => {
        ...
        jest.advanceTimersByTime(5001); // 5.001 s — expired
    ...
    test("SET with new TTL on an existing key resets the expiry", () => {
        const cache = makeCache(10);
        cache.set("k", "v1", 2);
        jest.advanceTimersByTime(1000); // 1 s elapsed
        // Re-set the same key with a fresh 5-second TTL
        cache.set("k", "v2", 5);
        jest.advanceTimersByTime(2001); // would have expired under old TTL
...
describe("TTL and LRU interaction", () => {
    ...
    test("LRU eviction targets LRU live node; expired nodes purged by sweep", () => {
        ...
        jest.advanceTimersByTime(1500); // 1.5 s — 'a' expired, sweep ran at 500 ms and 1000 ms
    ...
    test("updating a key's TTL via SET prevents it from being lazily expired", () => {
        ...
        cache.set("k", "v2", 5);
        jest.advanceTimersByTime(2001); // original TTL would have fired here
